<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ArticleEdit.aspx.cs" Inherits="FC.Admin.ArticleEdit" %>
...
<body  style="padding-top:0px;min-height:800px;min-width:800px;">
    <div class="container" id="page_main_content" style=" margin-left:0px;">
        ...
        <div class="container-fluid">
            <% if (isIdenUser && (user.type & 8) != 0 && articleLoad)
                //if(true)
               { %>
	        <div class="row-fluid">
            ...
            <div class="row-fluid">
                <div class="form-group form-group-lg">
                    <h4>标题&nbsp;&nbsp;<input class="form-control" type="text" id="article_title" placeholder="链接处所显示的内容" 
                    value="<%=System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(article.title))%>"/></h4>
                    ...
                    <select style="width:auto;" id="super_content_type" onchange="chageSuperContentType();">
                        <% 
                            ...
                            {
                                if (pair.Key.ToString() == article.contentTypeParent)
                                {
                    ...
                    <select style="width:auto;" id="content_type">
                        <% 
                            Dictionary<int, string> ContentType = FC.cjstudio.getContentType(int.Parse(article.contentTypeParent));
                            foreach (KeyValuePair<int ,string> pair in ContentType)
                            {
                                if (pair.Key.ToString() == article.contentType)
                                {
                                    Response.Write("<option selected=\"selected\" value=\"" +
                                           pair.Key + "\">" +
                                           pair.Value + "</option>");
	            ...
	            <div class="html_edit">
                    ...
                    <%
                        if (article.content != "0")
                        {
                            try
                            {
                                Response.Write( System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(article.content)));
                            }
                            catch (Exception)
                            {
                                ;
            ...
            <div>
                ...
                <input type="checkbox" id="show_at_home" 
                <% if((article.status &8)!=0)
            ...
            <% } else if (!articleLoad)
               { %>
							<div class="alert alert-error">
								...
								<h4>
									提示!
								</h4> <strong>异常!</strong> 加载文章出现未知错误.
							</div>
              <% } %>
		</div>
    </div>
